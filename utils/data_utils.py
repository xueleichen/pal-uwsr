...
class dataLoaderUSR():
    def __init__(self, DATA_PATH, SCALE=4):
        # SCALE = 2 (80, 60)  => (160,120)
        # SCALE = 4 (80, 60)  => (320,240)
    ...
    def get_lr_hr_paths(self, data_dir):
        ...
        for f in lr_path[:num_paths]:
            all_lr_paths.append(os.path.join(data_dir + "lr_8x/", f))
            all_hr_paths.append(os.path.join(data_dir + hr_fold, f))
...
class dataLoaderUSR4():
    def __init__(self, DATA_PATH, SCALE=4):
        # SCALE = 4 (160, 120)  => (320,240)
        # SCALE = 8 (160, 120)  => (640,480)  
        self.SCALE = SCALE
    ...
    def get_lr_hr_paths(self, data_dir):
        ...
        lr_path = sorted(os.listdir(data_dir+"lr_4x/"))
        if (self.SCALE==2):
            hr_fold = "lr_2x/"
            self.hr_res = (240,320)
        elif (self.SCALE==4):
        ...
        all_lr_paths, all_hr_paths = [], []
        for f in lr_path[:num_paths]:
            all_lr_paths.append(os.path.join(data_dir + "lr_4x/", f))
            all_hr_paths.append(os.path.join(data_dir + hr_fold, f))
        return num_paths, all_lr_paths, all_hr_paths
    ...
    def load_batch(self, batch_size=1, data_augment=True):
        ...
        for i in range(self.n_batches-1):
            ...
            imgs_lr, imgs_hr = [], []
            for idx in range(len(batch_lr)): 
                img_lr, img_hr =  read_and_resize_pair(batch_lr[idx], batch_hr[idx], low_res=(120,160), high_res=self.hr_res)
                if (data_augment): img_lr, img_hr = augment(img_lr, img_hr)
                imgs_lr.append(img_lr)
                imgs_hr.append(img_hr)
            imgs_lr = preprocess(np.array(imgs_lr))
    ...
    def load_val_data(self, batch_size=2):
        ...
        imgs_lr, imgs_hr = [], []
        for idx in range(len(paths_hr)):
            img_lr, img_hr = read_and_resize_pair(paths_lr[idx], paths_hr[idx], low_res=(120,160), high_res=self.hr_res)
            imgs_lr.append(img_lr)
            imgs_hr.append(img_hr)
        imgs_lr = preprocess(np.array(imgs_lr))
        imgs_hr = preprocess(np.array(imgs_hr))
